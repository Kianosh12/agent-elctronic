...
import { Zap, Moon, Sun, BrainCircuit } from 'lucide-react';
...
const App: React.FC = () => {
  ...
  const handleCalculate = async () => {
    ...
    if (window.innerWidth < 1024) { // Changed to match lg breakpoint
        setTimeout(() => {
    ...
    try {
      ...
      // For now, we rely on the API working, or the user trying again.
      alert("خطا در ارتباط با هوش مصنوعی. لطفاً مجدداً تلاش کنید.");
    } finally {
      setIsLoading(false);
    }
  };
        ...
        <footer className="bg-white dark:bg-slate-900 border-t border-slate-200 dark:border-slate-800 py-6 md:py-8 mt-auto transition-colors duration-300 print:hidden">
          <div className="max-w-6xl mx-auto px-4 text-center">
              <p className="text-xs md:text-sm text-slate-500 dark:text-slate-400">
                  طراحی شده با ❤️ توسط ایجنت هوشمند Gemini | مبنای محاسبات: <span className="font-bold text-slate-700 dark:text-slate-300">تحلیل آنلاین بازار</span>
              </p>
          </div>
        </footer>
      </div>
